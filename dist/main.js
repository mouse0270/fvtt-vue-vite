...
const Module = foundry.utils.mergeObject(defaults, ModuleData);
...
function applyOptions(instance) {
  ...
  if (computedOptions) {
    for (const key in computedOptions) {
      ...
      if (!!(define_process_env_default$2.NODE_ENV !== "production") && get2 === NOOP) {
        warn$1(`Computed property "${key}" has no getter.`);
      }
      const set2 = !isFunction(opt) && isFunction(opt.set) ? opt.set.bind(publicThis) : !!(define_process_env_default$2.NODE_ENV !== "production") ? () => {
        warn$1(
...
const resolveTarget = (props, select) => {
  ...
  if (isString(targetSelector)) {
    ...
    if (!!(define_process_env_default$2.NODE_ENV !== "production") && !targetSelector && !isTeleportDisabled(props)) {
...
const TeleportImpl = {
  ...
  process(n1, n2, container, anchor, parentComponent, parentSuspense, namespace, slotScopeIds, optimized, internals) {
    ...
    if (n1 == null) {
      const placeholder = n2.el = !!(define_process_env_default$2.NODE_ENV !== "production") ? createComment("teleport start") : createText("");
      const mainAnchor = n2.anchor = !!(define_process_env_default$2.NODE_ENV !== "production") ? createComment("teleport end") : createText("");
      ...
      } else if (!!(define_process_env_default$2.NODE_ENV !== "production") && !disabled) {
  ...
  setup(props, { slots }) {
    ...
    return () => {
      ...
      for (let i = 0; i < children.length; i++) {
        ...
        } else if (!!(define_process_env_default$1.NODE_ENV !== "production")) {
...
function VueApplicationMixin(BaseApplication) {
  class VueApplication2 extends BaseApplication {
    /**
    ...
    async _renderHTML(context, options) {
      ...
      if (this.constructor.DEBUG)
        console.log(`VueApplicationMixin | _renderHTML |`, context, options);
      for (const partId of options.parts) {
        ...
        if (!this.#props?.[partId])
          this.#props[partId] = reactive(options?.props ?? part?.props ?? {});
        else
          foundry.utils.mergeObject(this.#props[partId], options?.props ?? {}, { inplace: true, insertKeys: true });
        rendered[partId] = await (part?.app ?? part?.component ?? part?.template);
      }
      if (this.constructor.DEBUG)
        console.log(`VueApplicationMixin | _renderHTML | Vue Instances |`, this.#instance);
      return rendered;
    }
    /**
    ...
    _replaceHTML(result, content, options) {
      if (this.constructor.DEBUG)
        console.log(`VueApplicationMixin | _replaceHTML |`, result, content, options);
      if (!this.#instance) {
        const Instance = this;
        this.#instance = createApp({
          render: () => Object.entries(result).map(
            ([key, value]) => h("div", {
    ...
    async #onSubmitForm(htmlElement, event) {
      ...
      const part = this.constructor.PARTS[partId];
      if (!part?.forms)
        return console.warn("VueApplicationMixin | onSubmitForm | No forms found for part", partId);
    ...
    #onChangeForm(htmlElement, event) {
      ...
      const part = this.constructor.PARTS[partId];
      if (!part?.forms)
        return console.warn("VueApplicationMixin | onChangeForm | No forms found for part", partId);
      for (const [selector, formConfig] of Object.entries(part.forms)) {
        const form = htmlElement.matches(selector) ? htmlElement : htmlElement.querySelector(selector);
        if (!form)
          return;
        if (formConfig?.submitOnChange)
          this.#onSubmitForm(htmlElement, event);
      }
    }
  }
  return VueApplication2;
}
function log(type, ...args) {
  if (!Module?.flags?.logger?.enabled)
    return;
  console[type](
    `%c${Module.title}`,
...
const _sfc_main$l = {
  ...
  setup(__props) {
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock("h1", null, toDisplayString(message.value), 1);
    };
  }
};
  ...
  setup(__props) {
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock(Fragment, null, [
        createBaseVNode("h1", null, toDisplayString(message.value), 1),
        createBaseVNode("button", { onClick: reverseMessage }, "Reverse Message"),
        createBaseVNode("button", {
          onClick: _cache[0] || (_cache[0] = ($event) => message.value += "!")
        }, 'Append "!"'),
        createBaseVNode("a", {
          href: "https://vuejs.org",
          onClick: withModifiers(notify, ["prevent"])
        }, " A link with e.preventDefault() ")
      ], 64);
    };
  }
};
const ExampleBasicHandlingUserInput = /* @__PURE__ */ _export_sfc(_sfc_main$k, [["__scopeId", "data-v-1f43b5dc"]]);
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock(Fragment, null, [
        createBaseVNode("p", null, [
          createBaseVNode("span", { title: message.value }, " Hover your mouse over me for a few seconds to see my dynamically bound title! ", 8, _hoisted_1$f)
        ]),
        createBaseVNode("p", {
          class: normalizeClass({ red: isRed.value }),
          onClick: toggleRed
        }, " This should be red... but click me to toggle it. ", 2),
        createBaseVNode("p", {
          style: normalizeStyle({ color: color.value }),
          onClick: toggleColor
        }, " This should be green, and should toggle between green and blue on click. ", 4)
      ], 64);
    };
  }
};
const ExampleBasicAttributeBindings = /* @__PURE__ */ _export_sfc(_sfc_main$j, [["__scopeId", "data-v-a03658bd"]]);
  ...
  setup(__props) {
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock(Fragment, null, [
        ...
        createBaseVNode("button", {
          onClick: _cache[1] || (_cache[1] = ($event) => list.value.push(list.value.length + 1))
        }, "Push Number"),
        createBaseVNode("button", {
          onClick: _cache[2] || (_cache[2] = ($event) => list.value.pop())
        }, "Pop Number"),
        createBaseVNode("button", {
          onClick: _cache[3] || (_cache[3] = ($event) => list.value.reverse())
        }, "Reverse List"),
        show.value && list.value.length ? (openBlock(), createElementBlock("ul", _hoisted_1$e, [
        ...
        ])) : list.value.length ? (openBlock(), createElementBlock("p", _hoisted_2$b, "List is not empty, but hidden.")) : (openBlock(), createElementBlock("p", _hoisted_3$a, "List is empty."))
      ], 64);
    };
  }
};
const _hoisted_1$d = /* @__PURE__ */ createBaseVNode("h2", null, "Text Input", -1);
const _hoisted_2$a = /* @__PURE__ */ createBaseVNode("h2", null, "Checkbox", -1);
const _hoisted_3$9 = { for: "checkbox" };
const _hoisted_4$6 = /* @__PURE__ */ createBaseVNode("h2", null, "Multi Checkbox", -1);
const _hoisted_5$5 = /* @__PURE__ */ createBaseVNode("label", { for: "jack" }, "Jack", -1);
const _hoisted_6$3 = /* @__PURE__ */ createBaseVNode("label", { for: "john" }, "John", -1);
const _hoisted_7$2 = /* @__PURE__ */ createBaseVNode("label", { for: "mike" }, "Mike", -1);
const _hoisted_8$2 = /* @__PURE__ */ createBaseVNode("h2", null, "Radio", -1);
const _hoisted_9$2 = /* @__PURE__ */ createBaseVNode("label", { for: "one" }, "One", -1);
const _hoisted_10$1 = /* @__PURE__ */ createBaseVNode("br", null, null, -1);
const _hoisted_11$1 = /* @__PURE__ */ createBaseVNode("label", { for: "two" }, "Two", -1);
const _hoisted_12$1 = /* @__PURE__ */ createBaseVNode("br", null, null, -1);
const _hoisted_13$1 = /* @__PURE__ */ createBaseVNode("h2", null, "Select", -1);
const _hoisted_14$1 = /* @__PURE__ */ createBaseVNode("option", {
  disabled: "",
  value: ""
}, "Please select one", -1);
const _hoisted_15$1 = /* @__PURE__ */ createBaseVNode("option", null, "A", -1);
const _hoisted_16 = /* @__PURE__ */ createBaseVNode("option", null, "B", -1);
const _hoisted_17 = /* @__PURE__ */ createBaseVNode("option", null, "C", -1);
const _hoisted_18 = [
  _hoisted_14$1,
  _hoisted_15$1,
  _hoisted_16,
...
const _hoisted_19 = /* @__PURE__ */ createBaseVNode("h2", null, "Multi Select", -1);
const _hoisted_20 = /* @__PURE__ */ createBaseVNode("option", null, "A", -1);
const _hoisted_21 = /* @__PURE__ */ createBaseVNode("option", null, "B", -1);
const _hoisted_22 = /* @__PURE__ */ createBaseVNode("option", null, "C", -1);
const _hoisted_23 = [
  _hoisted_20,
  _hoisted_21,
  _hoisted_22
  ...
  setup(__props) {
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock(Fragment, null, [
        ...
        createBaseVNode("label", _hoisted_3$9, "Checked: " + toDisplayString(checked.value), 1),
        _hoisted_4$6,
        withDirectives(createBaseVNode("input", {
          type: "checkbox",
          id: "jack",
          value: "Jack",
          "onUpdate:modelValue": _cache[2] || (_cache[2] = ($event) => checkedNames.value = $event)
        }, null, 512), [
          [vModelCheckbox, checkedNames.value]
        ]),
        _hoisted_5$5,
        withDirectives(createBaseVNode("input", {
          type: "checkbox",
          id: "john",
          value: "John",
          "onUpdate:modelValue": _cache[3] || (_cache[3] = ($event) => checkedNames.value = $event)
        }, null, 512), [
          [vModelCheckbox, checkedNames.value]
        ]),
        _hoisted_6$3,
        withDirectives(createBaseVNode("input", {
          ...
          id: "mike",
          value: "Mike",
          "onUpdate:modelValue": _cache[4] || (_cache[4] = ($event) => checkedNames.value = $event)
        }, null, 512), [
          [vModelCheckbox, checkedNames.value]
        ]),
        _hoisted_7$2,
        createBaseVNode("p", null, [
          createTextVNode("Checked names: "),
          createBaseVNode("pre", null, toDisplayString(checkedNames.value), 1)
        ]),
        _hoisted_8$2,
        withDirectives(createBaseVNode("input", {
          type: "radio",
        ...
        withDirectives(createBaseVNode("input", {
          ...
          id: "two",
          value: "Two",
          "onUpdate:modelValue": _cache[6] || (_cache[6] = ($event) => picked.value = $event)
        }, null, 512), [
          [vModelRadio, picked.value]
        ]),
        _hoisted_11$1,
        _hoisted_12$1,
        createBaseVNode("span", null, "Picked: " + toDisplayString(picked.value), 1),
        _hoisted_13$1,
        ...
        createBaseVNode("span", null, "Selected: " + toDisplayString(selected.value), 1),
        _hoisted_19,
        withDirectives(createBaseVNode("select", {
          "onUpdate:modelValue": _cache[8] || (_cache[8] = ($event) => multiSelected.value = $event),
          multiple: "",
          style: { "width": "100px" }
        }, _hoisted_23, 512), [
          [vModelSelect, multiSelected.value]
        ]),
        createBaseVNode("span", null, "Selected: " + toDisplayString(multiSelected.value), 1)
      ], 64);
    };
  }
};
const _sfc_main$g = {
  ...
  setup(__props) {
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock("li", null, toDisplayString(__props.todo.text), 1);
    };
  }
};
const _sfc_main$f = {
  ...
  setup(__props) {
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock("ol", null, [
        (openBlock(true), createElementBlock(Fragment, null, renderList(groceryList.value, (item) => {
...
const _hoisted_1$c = { class: "editor" };
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock("div", _hoisted_1$c, [
        createBaseVNode("textarea", {
          class: "input",
          value: input.value,
          onInput: _cache[0] || (_cache[0] = (...args) => unref(update) && unref(update)(...args))
...
const ExamplePracticalMarkdownEditor = /* @__PURE__ */ _export_sfc(_sfc_main$e, [["__scopeId", "data-v-5ebb6791"]]);
const _withScopeId$2 = (n) => (pushScopeId("data-v-384e13c0"), n = n(), popScopeId(), n);
const _hoisted_1$b = /* @__PURE__ */ _withScopeId$2(() => /* @__PURE__ */ createBaseVNode("h1", null, "Latest Vue Core Commits", -1));
const _hoisted_2$8 = ["id", "value"];
const _hoisted_3$7 = ["for"];
const _hoisted_4$5 = ["href"];
const _hoisted_5$4 = { class: "message" };
const _hoisted_6$2 = /* @__PURE__ */ _withScopeId$2(() => /* @__PURE__ */ createBaseVNode("br", null, null, -1));
const _hoisted_7$1 = { class: "author" };
const _hoisted_8$1 = ["href"];
const _hoisted_9$1 = { class: "date" };
...
const _sfc_main$d = {
  ...
  setup(__props) {
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock(Fragment, null, [
        _hoisted_1$b,
        (openBlock(), createElementBlock(Fragment, null, renderList(branches, (branch) => {
          return openBlock(), createElementBlock(Fragment, null, [
            withDirectives(createBaseVNode("input", {
              type: "radio",
              id: branch,
              value: branch,
              name: "branch",
              "onUpdate:modelValue": _cache[0] || (_cache[0] = ($event) => currentBranch.value = $event)
            }, null, 8, _hoisted_2$8), [
              [vModelRadio, currentBranch.value]
            ]),
            createBaseVNode("label", { for: branch }, toDisplayString(branch), 9, _hoisted_3$7)
          ], 64);
        }), 64)),
        createBaseVNode("p", null, "vuejs/vue@" + toDisplayString(currentBranch.value), 1),
        createBaseVNode("ul", null, [
          (openBlock(true), createElementBlock(Fragment, null, renderList(commits.value, ({ html_url, sha, author, commit }) => {
            return openBlock(), createElementBlock("li", null, [
              createBaseVNode("a", {
                href: html_url,
                target: "_blank",
                class: "commit"
              }, toDisplayString(sha.slice(0, 7)), 9, _hoisted_4$5),
              createTextVNode(" - "),
              createBaseVNode("span", _hoisted_5$4, toDisplayString(truncate(commit.message)), 1),
              _hoisted_6$2,
              createTextVNode(" by "),
              createBaseVNode("span", _hoisted_7$1, [
                createBaseVNode("a", {
                  ...
                  target: "_blank"
                }, toDisplayString(commit.author.name), 9, _hoisted_8$1)
              ]),
              createTextVNode(" at "),
              createBaseVNode("span", _hoisted_9$1, toDisplayString(formatDate(commit.author.date)), 1)
            ]);
          }), 256))
        ])
      ], 64);
...
const ExamplePracticalFetchingData = /* @__PURE__ */ _export_sfc(_sfc_main$d, [["__scopeId", "data-v-384e13c0"]]);
...
const _sfc_main$c = {
  ...
  setup(__props) {
    ...
    const filteredData = computed(() => {
      ...
      if (filterKey) {
        ...
        data = data.filter((row) => {
          return Object.keys(row).some((key2) => {
            return String(row[key2]).toLowerCase().indexOf(filterKey) > -1;
          });
        });
      }
    ...
    return (_ctx, _cache) => {
      return filteredData.value.length ? (openBlock(), createElementBlock("table", _hoisted_1$a, [
        createBaseVNode("thead", null, [
          createBaseVNode("tr", null, [
            (openBlock(true), createElementBlock(Fragment, null, renderList(__props.columns, (key) => {
              return openBlock(), createElementBlock("th", {
                onClick: ($event) => sortBy(key),
                class: normalizeClass({ active: sortKey.value == key })
              }, [
                createTextVNode(toDisplayString(capitalize2(key)) + " ", 1),
                createBaseVNode("span", {
                  class: normalizeClass(["arrow", sortOrders.value[key] > 0 ? "asc" : "dsc"])
                }, null, 2)
              ], 10, _hoisted_2$7);
            }), 256))
          ])
        ...
        createBaseVNode("tbody", null, [
          (openBlock(true), createElementBlock(Fragment, null, renderList(filteredData.value, (entry) => {
            return openBlock(), createElementBlock("tr", null, [
              (openBlock(true), createElementBlock(Fragment, null, renderList(__props.columns, (key) => {
                return openBlock(), createElementBlock("td", null, toDisplayString(entry[key]), 1);
              }), 256))
            ]);
          }), 256))
        ])
      ])) : (openBlock(), createElementBlock("p", _hoisted_3$6, "No matches found."));
    };
  }
};
const DemoGrid$1 = /* @__PURE__ */ _export_sfc(_sfc_main$c, [["__scopeId", "data-v-eb4f16b7"]]);
const _hoisted_1$9 = { id: "search" };
const _sfc_main$b = {
  __name: "App",
  setup(__props) {
    ...
    const gridData = [
      ...
      { name: "Bruce Lee", power: 9e3 },
      { name: "Jackie Chan", power: 7e3 },
      { name: "Jet Li", power: 8e3 }
    ];
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock(Fragment, null, [
        createBaseVNode("form", _hoisted_1$9, [
...
const _sfc_main$a = {
  ...
  setup(__props) {
    ...
    return (_ctx, _cache) => {
      ...
      return openBlock(), createElementBlock("li", null, [
        createBaseVNode("div", {
          class: normalizeClass({ bold: isFolder.value }),
          onClick: toggle,
          onDblclick: changeType
        }, [
          createTextVNode(toDisplayString(__props.model.name) + " ", 1),
          isFolder.value ? (openBlock(), createElementBlock("span", _hoisted_1$8, "[" + toDisplayString(isOpen.value ? "-" : "+") + "]", 1)) : createCommentVNode("", true)
        ], 34),
        isFolder.value ? withDirectives((openBlock(), createElementBlock("ul", _hoisted_2$6, [
          (openBlock(true), createElementBlock(Fragment, null, renderList(__props.model.children, (model) => {
...
const _hoisted_2$5 = { class: "modal-container" };
const _hoisted_3$5 = { class: "modal-header" };
const _hoisted_4$4 = { class: "modal-body" };
const _hoisted_5$3 = { class: "modal-footer" };
const _sfc_main$8 = {
  __name: "Modal",
  props: {
    show: Boolean
            ...
            createBaseVNode("div", _hoisted_2$5, [
              ...
              createBaseVNode("div", _hoisted_5$3, [
                renderSlot(_ctx.$slots, "footer", {}, () => [
                  createTextVNode(" default footer "),
                  createBaseVNode("button", {
...
const Modal = /* @__PURE__ */ _export_sfc(_sfc_main$8, [["__scopeId", "data-v-845e2820"]]);
const _hoisted_1$6 = /* @__PURE__ */ createBaseVNode("h3", null, "custom header", -1);
const _sfc_main$7 = {
  __name: "App",
  setup(__props) {
      ...
      return openBlock(), createElementBlock(Fragment, null, [
        ...
        (openBlock(), createBlock(Teleport, { to: "body" }, [
          createVNode(Modal, {
            ...
            onClose: _cache[1] || (_cache[1] = ($event) => showModal.value = false)
          }, {
            header: withCtx(() => [
              _hoisted_1$6
            ]),
...
const _sfc_main$6 = {
  ...
  setup(__props) {
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock(Fragment, null, [
        createBaseVNode("button", { onClick: insert }, "insert at random index"),
        createBaseVNode("button", { onClick: reset }, "reset"),
        createBaseVNode("button", { onClick: shuffle }, "shuffle"),
...
const ExamplePracticalListWithTransitions = /* @__PURE__ */ _export_sfc(_sfc_main$6, [["__scopeId", "data-v-7d8a570a"]]);
const _withScopeId$1 = (n) => (pushScopeId("data-v-6f3c918b"), n = n(), popScopeId(), n);
const _hoisted_1$4 = { class: "todoapp" };
const _hoisted_2$4 = { class: "header" };
const _hoisted_3$4 = /* @__PURE__ */ _withScopeId$1(() => /* @__PURE__ */ createBaseVNode("h1", null, "todos", -1));
const _hoisted_4$3 = { class: "main" };
const _hoisted_5$2 = ["checked"];
const _hoisted_6$1 = /* @__PURE__ */ _withScopeId$1(() => /* @__PURE__ */ createBaseVNode("label", { for: "toggle-all" }, "Mark all as complete", -1));
...
const _sfc_main$5 = {
  ...
  setup(__props) {
    const filters = {
      all: (todos2) => todos2,
      active: (todos2) => todos2.filter((todo) => !todo.completed),
      completed: (todos2) => todos2.filter((todo) => todo.completed)
    };
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock("section", _hoisted_1$4, [
        ...
        withDirectives(createBaseVNode("section", _hoisted_4$3, [
          createBaseVNode("input", {
            id: "toggle-all",
            class: "toggle-all",
            type: "checkbox",
          ...
          createBaseVNode("ul", _hoisted_7, [
            (openBlock(true), createElementBlock(Fragment, null, renderList(filteredTodos.value, (todo) => {
              return openBlock(), createElementBlock("li", {
                class: normalizeClass(["todo", { completed: todo.completed, editing: todo === editedTodo.value }]),
                key: todo.id
              }, [
                createBaseVNode("div", _hoisted_8, [
                  ...
                  }, toDisplayString(todo.title), 41, _hoisted_10),
                  createBaseVNode("button", {
                    class: "destroy",
                    onClick: ($event) => removeTodo(todo)
                  }, null, 8, _hoisted_11)
                ]),
                todo === editedTodo.value ? withDirectives((openBlock(), createElementBlock("input", {
                  key: 0,
                  class: "edit",
                  type: "text",
                  ...
                  onKeyup: [
                    withKeys(($event) => doneEdit(todo), ["enter"]),
                    withKeys(($event) => cancelEdit(todo), ["escape"])
                  ]
                }, null, 40, _hoisted_12)), [
          ...
          [vShow, todos.value.length]
        ]),
        withDirectives(createBaseVNode("footer", _hoisted_13, [
          createBaseVNode("span", _hoisted_14, [
            createBaseVNode("strong", null, toDisplayString(remaining.value), 1),
            createBaseVNode("span", null, toDisplayString(remaining.value === 1 ? " item" : " items") + " left", 1)
          ]),
          createBaseVNode("ul", _hoisted_15, [
            createBaseVNode("li", null, [
              createBaseVNode("a", {
                href: "#/all",
                class: normalizeClass({ selected: visibility.value === "all" })
              }, "All", 2)
            ]),
            createBaseVNode("li", null, [
              createBaseVNode("a", {
                href: "#/active",
                class: normalizeClass({ selected: visibility.value === "active" })
              }, "Active", 2)
            ]),
            createBaseVNode("li", null, [
              createBaseVNode("a", {
                href: "#/completed",
                class: normalizeClass({ selected: visibility.value === "completed" })
              }, "Completed", 2)
            ])
          ]),
          withDirectives(createBaseVNode("button", {
            ...
            [vShow, todos.value.length > remaining.value]
          ])
        ], 512), [
          [vShow, todos.value.length]
        ])
...
const ExamplePracticalTodoMVC = /* @__PURE__ */ _export_sfc(_sfc_main$5, [["__scopeId", "data-v-6f3c918b"]]);
            ...
            (openBlock(true), createElementBlock(Fragment, null, renderList(users.value, (user) => {
              return openBlock(), createElementBlock("li", {
                ...
                createBaseVNode("div", null, [
                  createBaseVNode("h3", {
                    style: normalizeStyle(`${user.active ? `--player-color: ${user.color.css}` : ""}`)
                  }, [
                    createTextVNode(toDisplayString(user.name) + " ", 1),
                    user.pronouns.length > 0 ? (openBlock(), createElementBlock("span", _hoisted_2$3, "(" + toDisplayString(user.pronouns) + ")", 1)) : createCommentVNode("", true)
                  ], 4),
                  user?.character ? (openBlock(), createElementBlock("p", _hoisted_3$3, [
                    createBaseVNode("strong", null, "[" + toDisplayString(user.character.type.toUpperCase()) + "]", 1),
                    createTextVNode(" " + toDisplayString(user.character.name), 1)
                  ])) : (openBlock(), createElementBlock("p", _hoisted_4$2, toDisplayString(unref(localize)(`${unref(Module).id}.EXAMPLES.FOUNDRY_VTT.PLAYERS_LANG.no_active_character`)), 1))
                ])
              ]);
            }), 128))
          ]),
...
const ExampleFoundryVTTPlayers = /* @__PURE__ */ _export_sfc(_sfc_main$4, [["__scopeId", "data-v-0a8d8993"]]);
const _hoisted_1$2 = { class: "app" };
const _hoisted_2$2 = { class: "sidebar" };
const _hoisted_3$2 = ["onClick"];
const _hoisted_4$1 = { class: "main" };
const _hoisted_5$1 = { class: "component" };
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock("div", _hoisted_1$2, [
        createBaseVNode("div", _hoisted_2$2, [
          createBaseVNode("ul", null, [
            (openBlock(true), createElementBlock(Fragment, null, renderList(Object.keys(sidebar), (category2) => {
              return openBlock(), createElementBlock("li", { key: category2 }, [
                createBaseVNode("h3", null, toDisplayString(unref(localize)(`${unref(Module).id}.EXAMPLES.${category2}.TITLE`)), 1),
                createBaseVNode("ul", null, [
                  (openBlock(true), createElementBlock(Fragment, null, renderList(Object.keys(sidebar[category2]), (item) => {
                    return openBlock(), createElementBlock("li", {
                      key: item,
                      onClick: ($event) => selected({ category: category2, name: item })
                    }, toDisplayString(unref(localize)(`${unref(Module).id}.EXAMPLES.${category2}.${item}`)), 9, _hoisted_3$2);
                  }), 128))
                ])
              ]);
            }), 128))
        ...
        createBaseVNode("div", _hoisted_4$1, [
          createBaseVNode("h1", null, toDisplayString(unref(localize)(`${unref(Module).id}.EXAMPLES.${category.value}.TITLE`)) + " - " + toDisplayString(unref(localize)(`${unref(Module).id}.EXAMPLES.${category.value}.${example.value}`)), 1),
          createBaseVNode("div", _hoisted_5$1, [
            (openBlock(), createBlock(resolveDynamicComponent(selectedComponent.value)))
          ]),
          createBaseVNode("footer", null, [
            [`BASIC`, `PRACTICAL`].includes(category.value) ? (openBlock(), createElementBlock("p", {
              key: 0,
              innerHTML: unref(localize)(`${unref(Module).id}.EXAMPLES.FOOTER.PROVIDED_BY`, {
                source: [`BASIC`, `PRACTICAL`].includes(category.value) ? "Vue.js" : "Mouse0270",
                url: footerLinks[category.value][example.value],
                category: unref(localize)(`${unref(Module).id}.EXAMPLES.${category.value}.TITLE`),
                example: unref(localize)(`${unref(Module).id}.EXAMPLES.${category.value}.${example.value}`)
              })
...
const App = /* @__PURE__ */ _export_sfc(_sfc_main$3, [["__scopeId", "data-v-80a77d36"]]);
const { ApplicationV2: ApplicationV2$1 } = foundry.applications.api;
class VueExamples extends VueApplicationMixin(ApplicationV2$1) {
...
const _sfc_main$2 = {
  ...
  setup(__props) {
    ...
    return (_ctx, _cache) => {
      return filteredData.value.length ? (openBlock(), createElementBlock("table", _hoisted_1$1, [
        createBaseVNode("thead", null, [
          createBaseVNode("tr", null, [
            (openBlock(true), createElementBlock(Fragment, null, renderList(__props.columns, (key) => {
              return openBlock(), createElementBlock("th", {
      ...
      ])) : (openBlock(), createElementBlock("p", _hoisted_3$1, "No matches found."));
    };
  }
};
const DemoGrid = /* @__PURE__ */ _export_sfc(_sfc_main$2, [["__scopeId", "data-v-9f784e77"]]);
const _sfc_main$1 = {
  __name: "App",
  props: {
    title: {
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock(Fragment, null, [
        ...
        createBaseVNode("form", {
          class: "form1",
          id: "search",
          onInput: _cache[1] || (_cache[1] = (...args) => unref(onInput) && unref(onInput)(...args)),
          onChange: _cache[2] || (_cache[2] = (...args) => unref(onChange) && unref(onChange)(...args)),
          onSubmit: _cache[3] || (_cache[3] = (...args) => unref(onSubmit) && unref(onSubmit)(...args))
        }, [
          createTextVNode(" Search "),
          withDirectives(createBaseVNode("input", {
            name: "query",
        ...
        createVNode(DemoGrid, {
          data: gridData,
          columns: gridColumns,
          "filter-key": searchQuery.value
        }, null, 8, ["filter-key"]),
        createBaseVNode("button", {
          onClick: _cache[4] || (_cache[4] = (...args) => unref(buttonAction1) && unref(buttonAction1)(...args))
        }, "Button 1"),
        createBaseVNode("button", {
          onClick: _cache[5] || (_cache[5] = (...args) => unref(buttonAction2) && unref(buttonAction2)(...args))
        }, "Button 2")
      ], 64);
    };
  }
};
const _withScopeId = (n) => (pushScopeId("data-v-0b8cab77"), n = n(), popScopeId(), n);
const _hoisted_1 = { class: "standard-form" };
const _hoisted_2 = ["innerHTML"];
const _hoisted_3 = { class: "form-footer" };
const _hoisted_4 = { type: "submit" };
const _hoisted_5 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("i", { class: "fas fa-save" }, null, -1));
const _sfc_main = {
  __name: "UserConfig",
  setup(__props) {
    const fields = ref({
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock("div", _hoisted_1, [
        createBaseVNode("fieldset", null, [
          createBaseVNode("legend", null, toDisplayString(unref(localize)("USER.SHEET.HEADERS.INFO")), 1),
          (openBlock(true), createElementBlock(Fragment, null, renderList(fields.value, (field, key) => {
            return openBlock(), createElementBlock("div", {
              key,
              innerHTML: unref(formField)(field.field, field.options)
            }, null, 8, _hoisted_2);
          }), 128))
        ]),
        createBaseVNode("fieldset", null, [
          createBaseVNode("legend", null, toDisplayString(unref(localize)("USER.SHEET.HEADERS.CHAR")), 1)
        ]),
        createBaseVNode("footer", _hoisted_3, [
          createBaseVNode("button", _hoisted_4, [
            _hoisted_5,
            createBaseVNode("label", null, toDisplayString(unref(localize)("USER.SHEET.BUTTONS.SAVE")), 1)
          ])
        ])
      ]);
    };
...
const UserConfig = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-0b8cab77"]]);
...
class VueApplication extends VueApplicationMixin(ApplicationV2) {
      ...
      forms: {
        ".form1": {
          ...
          handler(event, form, formData) {
            logger.log("Form1 |", this, event, form, formData);
          }
        },
        ".form2": {
          handler(event, form, formData) {
            logger.log("Form2 |", this, event, form, formData);
          }
        }
      }
    }
  ...
  static #onClickButtonAction1(event, target) {
    logger.log("ButtonAction1 |", this, event);
  }
  static #onClickButtonAction2(event, target) {
    logger.log("ButtonAction2 |", this, event);
  }
}
Hooks.once("ready", async () => {
  logger.log("Hook.Ready | VITE Vue Application | Use ViteUserConfig() to render the Vue Sample usign VueApplicationMixin.");
  window.ViteUserConfig = async () => await new VueUserConfig().render(true);
  logger.log("Hook.Ready | VITE Vue Application | Use ViteApp() to render the Vue Sample using VueApplicationMixin.");
  window.ViteApp = async () => await new VueApplication().render(true);
  logger.log("Hook.Ready | VITE Vue Application | Use ViteExamples() to render the Vue Sample using VueApplicationMixin.");
